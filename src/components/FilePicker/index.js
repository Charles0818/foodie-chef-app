...
export const useMediaPermissons = () => {
  const Recording = async () => {
    const [recordingStatus, setRecordingStatus] = React.useState({
      canRecord: false, isDoneRecording: false, durationMillis: 0, URI: ''
    });
    React.useEffect(() => {
      const getPermissionAsync = async () => {
    ...
    try {
      ...
      // An error occurred!
      console.log(error);
      alert(`An error occured while trying to intitiate recording. Kindly try again.`);
    }
    ...
    return { recordingStatus, stopRecording, recording }
  }
  return { Recording,  }
};
